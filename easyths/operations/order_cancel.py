...
class OrderCancelOperation(BaseOperation):
    ...
    def execute(self, params: dict[str, Any]) -> OperationResult:
        ...
        try:
            self.logger.info("执行撤单操作", stock_code=stock_code or "全部")
            ...
            if cancel_type == "all":
                ...
                return OperationResult(
                    success=False, message=f"不支持的撤单类型: {cancel_type}"
                )
            ...
            is_op_success = not self.wait_for_pop_dialog(
                0.4
            )  # 没有弹窗就是成功了，这里其实多余判断，但是无所谓，不耗什么时间
            ...
            self.logger.info(
                f"撤单操作{'成功' if is_op_success else '失败'}，耗时{time.time() - start_time}, 操作结果：",
                **result_data,
            )
            return OperationResult(
                message=f"撤销{stock_code}的委托{'成功' if is_op_success else '失败'}"
                if stock_code
                else "已成功撤销所有委托",
