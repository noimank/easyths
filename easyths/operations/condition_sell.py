    ...
    def execute(self, params: dict[str, Any]) -> OperationResult:
        ...
        try:
            ...
            if pop_dialog_title == "ConditionToolBar":
                ...
                tab_control = self.get_control_with_children(
                    document_panel, control_type="Tab"
                )
                # 选择卖出
                # 获取所有子控件（即各个选项卡）
                ...
                if not next_btn.is_enabled():
                    ...
                    return OperationResult(
                        success=False,
                        message="条件卖出设置失败，请检查是否持有该标的",
                        data={"msg": "条件卖出设置失败，请检查是否持有该标的"},
                    )
                next_btn.click()
                # 等页面重绘渲染
                self.sleep(0.5)
            ...
            self.logger.info(
                f"条件卖出操作耗时{time.time() - start_time}, 操作结果：", **result_data
            )
            return OperationResult(
                message=op_message,
