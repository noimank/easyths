...
class SellOperation(BaseOperation):
    ...
    def execute(self, params: dict[str, Any]) -> OperationResult:
        ...
        try:
            self.logger.info(
                "执行卖出操作", stock_code=stock_code, price=price, quantity=quantity
            )
            ...
            # 二次确认
                ...
                message = "卖出操作未能成功，请检查软件设置是否有项目要求不符的地方"
                is_op_success = False
            ...
            self.logger.info(
                f"卖出操作{'成功' if is_op_success else '失败'}，耗时{time.time() - start_time}, 操作结果：",
                **result_data,
            )
