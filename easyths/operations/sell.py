...
class SellOperation(BaseOperation[LimitOrderParams]):
    """卖出股票操作"""
    ...
    def execute(self, params: LimitOrderParams) -> OperationResult:
        ...
        if not is_op_success:
            ...
            return self._fail(
                f"卖出提交被拒: {self.get_pop_dialog_content()}",
                ErrorCode.CLIENT_REJECTED,
            )
        if len(stock_name) > 0:
            # 表单未清空，说明委托没有真正提交
            return self._fail(
                "卖出操作未能成功，请检查软件设置是否有与项目要求不符的地方",
                ErrorCode.UI_ERROR,
            )
        ...
        return self._ok(
            ...
            message=f"成功提交{stock_code}的卖出委托，耗时{time.time() - start_time:.2f}秒",
        )
