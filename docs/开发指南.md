...
```python
from easyths.core.base_operation import BaseOperation
...
将插件文件放在 `easyths/operations/` 目录下，系统启动时会自动加载。
...
from easyths.operations.my_operation import MyOperation
...
async def test_validate_success(my_operation):
    ...
    result = my_operation.validate(params)
...
async def test_execute_success(my_operation):
    ...
    result = my_operation.execute(params)
